...
Mantid is used for reduction and analysis of neutron and muon data.
This section describes the architecture of the mantid software in terms
of the `C4 model <C4Model_>`_.

Context
-------

The mantid software interacts with people and other systems as described
by the following context diagram.
...
Experimental users of at neutron/muon facilities use the software
...
one general-purpose graphical application (``workbench``) and
...
- ``mantidqt`` *does not* depend on ``workbench``
...
``mantidqt`` is a Python package defined house a collcetion of Qt widgets
...
so guarantees of API stability.
